        ...
        if not devices:
            ...
            for dev in devices:
                ...
                cb = tk.Checkbutton(self.device_list, 
                                  ...
                                  variable=var)
                cb.pack(anchor='w')
                self.device_vars.append((var, dev['address']))
                
    def toggle_session(self, mode):
        if not self.running:
            # Connect selected HRV devices
            selected = [addr for var, addr in self.device_vars if var.get()]
            if selected:
        ...
        if self.running:
            ...
            if coherence_data:
                ...
                self.coherence_label.config(
                    text=f"Coherence: {avg_coherence:.3f} ({device_count} device{'s' if device_count != 1 else ''})"
                )
